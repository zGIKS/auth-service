...
impl SessionRepository for RedisSessionRepository {
    async fn create_session(&self, user_id: Uuid, jti: &str) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        let _: () = con.set_ex(key, jti, self.session_duration_seconds).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
    ...
    async fn save_refresh_token(&self, user_id: Uuid, refresh_token: &RefreshToken, ttl_seconds: u64) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        let _: () = con.set_ex(key.clone(), user_id.to_string(), ttl_seconds).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        
        // Also store a mapping of UserID -> Set of Refresh Token Hashes
        let user_tokens_key = format!("user_tokens:{}", user_id);
        let _: () = con.sadd(user_tokens_key.clone(), hashed_token).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        let _: () = con.expire(user_tokens_key, ttl_seconds as i64).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
    ...
    async fn delete_refresh_token(&self, refresh_token: &RefreshToken) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        if let Some(uid) = user_id_str {
            ...
            let _: () = con.srem(user_tokens_key, hashed_token).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        }

        Ok(())
    }

    async fn revoke_all_user_sessions(&self, user_id: Uuid) -> Result<(), Box<dyn Error + Send + Sync>> {
        let mut con = self.client.get_multiplexed_async_connection().await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        
        // 1. Blacklist the current active JTI
        let session_key = format!("session:{}", user_id);
        let current_jti: Option<String> = con.get(session_key.clone()).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        
        if let Some(jti) = current_jti {
            let blacklist_key = format!("blacklist:{}", jti);
            // Blacklist for the remaining session duration
            let _: () = con.set_ex(blacklist_key, "revoked", self.session_duration_seconds).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        }

        // 2. Get all refresh token hashes for the user
        let user_tokens_key = format!("user_tokens:{}", user_id);
        let token_hashes: Vec<String> = con.smembers(user_tokens_key.clone()).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        ...
        let _: () = con.del(user_tokens_key).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        
        // 5. Delete the active session key
        let _: () = con.del(session_key).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        ...
        let _: () = con.set_ex(invalidation_key, now, retention_ttl).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
    ...
    async fn is_jti_blacklisted(&self, jti: &str) -> Result<bool, Box<dyn Error + Send + Sync>> {
        ...
        let exists: bool = con.exists(key).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        Ok(exists)
    }

    async fn get_user_invalidation_timestamp(&self, user_id: Uuid) -> Result<Option<u64>, Box<dyn Error + Send + Sync>> {
        let mut con = self.client.get_multiplexed_async_connection().await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        let key = format!("invalidation_timestamp:{}", user_id);
        let timestamp_str: Option<String> = con.get(key).await.map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?;
        
        match timestamp_str {
            Some(s) => Ok(Some(s.parse::<u64>().map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)?)),
            None => Ok(None),
        }
    }
}
