use crate::iam::authentication::domain::{
    ...
    services::authentication_command_service::{AuthenticationQueryService, SessionRepository, TokenService},
};
use std::error::Error;
    ...
    S: SessionRepository,
{
    async fn verify_token(&self, token: &str) -> Result<Claims, Box<dyn Error + Send + Sync>> {
        ...
        if self.session_repository.is_jti_blacklisted(&claims.jti).await? {
            return Err("Token has been revoked".into());
        }

        // 3. Check if token was issued before the global invalidation timestamp
        if let Some(invalidation_timestamp) = self.session_repository.get_user_invalidation_timestamp(claims.sub).await? {
