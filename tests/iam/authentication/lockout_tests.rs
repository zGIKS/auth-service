use auth_service::shared::infrastructure::services::account_lockout::{AccountLockoutService, AccountLockoutVerifier};
use redis::Client;
...
async fn test_lockout_isolation_by_ip() {
    ...
    let _: () = service.reset_failure(email, Some(attacker_ip)).await.unwrap();
    ...
    for _ in 0..5 {
        service.register_failure(email, Some(attacker_ip), 5, 5).await.unwrap();
    }
    ...
    assert!(result.is_ok(), "Legitimate user should NOT be locked out despite attacker failures");
    ...
    assert!(result.is_ok(), "Global lock should not be engaged by IP-specific failures");
